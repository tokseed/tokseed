<h1 align="center">
  Hi there, I'm 
  <a href="https://Paulsthat.ru/" target="_blank" style="text-decoration: none; border-bottom: 2px solid #58a6ff;">
    Pavel
  </a>
</h1>

<h3 align="center" style="font-weight: 400; letter-spacing: -0.3px; color: #c9d1d9;">
  Computer science student · IT student from Russia 🇷🇺
</h3>

<div align="center" style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap; margin-top: 20px;">
  <img 
    src="https://media1.giphy.com/media/v1.Y2lkPTZjMDliOTUyN2JpZGp1em5pa3ozYThhOW83Z3RwMWxzcXN3N21jdmQ1a2p5a2UyaiZlcD12MV9naWZzX3NlYXJjaCZjdD1n/DSxKEQoQix9hC/giphy.gif" 
    width="280" 
    alt="coding gif"
  />
  <!-- REPLACE this URL with your working Steam image URL -->
  <img 
    src="https://your-working-url-here.png" 
    width="280" 
    alt="steam artwork"
  />
</div>


<p style="font-family: system-ui, -apple-system, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; font-size: 1.1rem;">
  Your text here
</p>
